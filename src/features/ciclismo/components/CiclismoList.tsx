...
interface CiclismoListProps {
  registros: Ciclismo[]
  onEdit: (registro: Ciclismo) => void
  onDelete: (registro: Ciclismo) => void
}
...
export function CiclismoList({ registros, onEdit, onDelete }: CiclismoListProps) {
  return (
        ...
        <li key={r.id} className="rounded-3xl border-2 border-line bg-white p-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <p className="flex items-center gap-2 text-sm font-extrabold text-ink">
              <Icon name="bici" size={16} className="text-primary" />
          ...
          <div className="grid grid-cols-2 gap-y-2">
            ...
            <Stat icon="velocidad" label="km/h" value={String(r.velocidadKmh)} />
